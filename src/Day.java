...
public class Day {
    ...
    public Event nextAvailableTimeSlot(int startTime, int duration) {
        ...
        for (int i = 0; i < events.size() - 1; i++) {
            if (events.get(i).getEndTime() + duration <= events.get(i + 1).getStartTime()
                    && startTime <= events.get(i).getEndTime()) {
                return new Event(events.get(i).getEndTime(),
                        events.get(i + 1).getStartTime() - events.get(i).getEndTime());
            }
